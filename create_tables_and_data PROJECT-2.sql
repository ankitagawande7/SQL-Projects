...
-- FETCHING SOME INFORMATION FROM DATABASE--
#Q(1)--List all artists for each record label name sorted by artist name.
select ar.name"artist name",
r.name"record label name" from record_label r join artist ar on r.id=ar.record_label_id order by ar.name asc;

#Q(2)--Which record labels have no artists?
select r.name"record lable name" from record_label r left join artist ar on r.id=ar.record_label_id 
where ar.record_label_id is null;

#Q(3)--List the number of songs per artist in descending order
select ar.name "artist name",
count(*) "number of Songs"
from song s join album al on s.album_id=al.id join artist ar on al.artist_id=ar.id group by ar.name order by count(*) desc;

select count(*)from song;

#Q(4)-- Which artist or artists have recorded the most number of songs?
select ar.name "artist name",
count(*) "number of song"
from song s join album al on s.album_id=al.id join artist ar on  artist_id=ar.id group by ar.name order by count(*) desc limit 1;-- (limit 1 for only one artist name)
...
select ar.name "artist name",
count(*) "number of song"
from song s join album al on s.album_id=al.id join artist ar on artist_id=ar.id group by ar.name having count(*)=(select min(numb_of_songs_list)
from(select ar.name artist_name_list,count(*) numb_of_songs_list from song s join album al on s.album_id=al.id join artist ar on artist_id=ar.id group by ar.name)temp1);
-- temp1 is tempr.variable to store data

#Q(6)--which artists have recorded songs longer than 5 minutes, and how many songs was that?
select temp.artist "artist name",
count(*) "number of song>5 mins"
from(select ar.name artist,s.duration "duration" from song s join album al on s.album_id=al.id join artist ar on al.artist_id=ar.id where duration>5)as temp group by temp.artist;
-- temp is tempr.variable to store data  

-- (HOW TO CROSS-CHECK THE ANSWER)
select duration,name from song where duration>5;
select count(*) from song where duration>5;
...
#Q(7)--list the artist, song and year of the  all longest recorded songs
select duration,name from song where duration>5;
select count(*) from song where duration>5;
...
#Q(8)--No. of Albums Recorded For Each Year.
select al.year"album year",
count(*) "Number of Album recorded" from album al group by al.year;

#Q(9)--Max Number of Record From Across all years?
select max(count)"maximum number of album recorded by year" from(
select al.year"album year",
count(*) count from album al group by al.year)as temp;

#Q(10)--List the Top 3 Artist with the longest Average Song Duration in Decreasing Order With Longest Average First?
select ar.name "artist name",
avg(s.duration)"average song duration"
from
    song s join album al on s.album_id = al.id
           join artist ar on al.artist_id = ar.id
           group by ar.name
